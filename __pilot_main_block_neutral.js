  try {
    if (typeof window.__markPilotMainReady === 'function') {
      window.__markPilotMainReady('pilot-main-script-top');
    } else if (window.__pilotDiag) {
      window.__pilotDiag.mainScriptReady = true;
      window.__pilotDiag.mainScriptReadyAt = Date.now();
      window.__pilotDiag.mainScriptStage = 'pilot-main-script-top-fallback';
    }
  } catch (_e) {}
    ...
    function lessonCollectEvaluation_() {
      ...
      document.querySelectorAll('.lesson-note-list').forEach(function(wrap, idx) {
        ...
        wrap.querySelectorAll('[data-lesson-note-item]').forEach(function(div) {
          items.push(String(div.textContent || '').replace(/^â€¢\s*/, '').trim());
        });
    ...
    function openLessonPlanModal_() {
      if (!_activeLessonContext) {
        if (window.M) M.toast({ html: 'Nenhum plano de aula ativo nesta missÃ£o.', classes: 'orange' });
        return;
      }
      var ctx = _activeLessonContext;
      ...
      sub.textContent = String(ctx.code || '') + ' â€¢ ' + String(ctx.title || 'Plano de Aula');
      content.innerHTML = ''
        + '<div style="font-size:0.8rem; line-height:1.6; color:#37474f; margin-bottom:10px;">'
        + '<b>Data:</b> ' + String(meta.date || mission.date || '') + ' &nbsp; <b>MatrÃ­cula:</b> ' + String(meta.acft || mission.acft || '') + ' &nbsp; <b>Piloto:</b> ' + String(meta.pilot || mission.pilot || '') + ' &nbsp; <b>Instrutor:</b> ' + String(meta.copilot || '')
        ...
        + '</div>'
        + '<h6 style="margin:8px 0 6px 0; color:#0b5394; font-weight:900;">AvaliaÃ§Ã£o de Pousos</h6>'
        + _lessonRenderGradeRows_(ctx.stops || [], 'landing')
        + '<h6 style="margin:12px 0 6px 0; color:#0b5394; font-weight:900;">AvaliaÃ§Ã£o de Manobras</h6>'
    ...
    function lessonAddInlineNote_(btn) {
      ...
      if (!wrap) return;
      var text = prompt('Digite a observaÃ§Ã£o:');
      if (text == null) return;
    ...
    function _dutyGetPositionContext_(snapshot) {
      return new Promise(function(resolve) {
        ...
        if (!(navigator && navigator.geolocation)) {
          resolve(Object.assign({}, base, { hint: 'GeolocalizaÃ§Ã£o indisponÃ­vel para validar geofence.' }));
          return;
        }
        navigator.geolocation.getCurrentPosition(function(pos) {
          ...
          if (!isFinite(lat) || !isFinite(lon)) {
            resolve(Object.assign({}, base, { hint: 'PosiÃ§Ã£o invÃ¡lida para validar geofence.' }));
            return;
          }
          ...
          const hint = nearest
            ? ('DistÃ¢ncia atÃ© geofence: ' + nearest.distKm.toFixed(2) + ' km (raio ' + radiusKm.toFixed(1) + ' km).')
            : 'NÃ£o foi possÃ­vel calcular distÃ¢ncia atÃ© a geofence.';
          ...
          resolve(Object.assign({}, base, { hint: 'PermissÃ£o de localizaÃ§Ã£o negada ou indisponÃ­vel.' }));
        }, {
    ...
    function _dutyGeoRenderVerifyMap_(geo) {
      ...
      _wpEnsureLeaflet(function() {
        ...
        _dutyGeoVerifyCurrentMarker = L.marker([geo.lat, geo.lon]).bindTooltip('Sua posiÃ§Ã£o').addTo(_dutyGeoVerifyMap);
    ...
    function _dutyOpenGeoVerifyModal_(geo) {
      return new Promise(function(resolve) {
        ...
        els.coords.textContent = 'Atual: ' + Number(geo.lat).toFixed(6) + ', ' + Number(geo.lon).toFixed(6)
          ...
          + ' | DistÃ¢ncia: ' + distText + ' km de ' + radiusText + ' km';
    ...
    async function _dutyPromptMorningStart_(ctx, snapshot, geo) {
      ...
      if (geo && geo.hasFence) {
        if (!geo.available) {
          return { prompted: false, blocked: 'location-unavailable', message: geo.hint || 'Sem localizaÃ§Ã£o para validar geofence.' };
        }
        if (!geo.inFence) {
        ...
        if (!verified) {
          return { prompted: false, blocked: 'map-cancel', message: 'VerificaÃ§Ã£o de posiÃ§Ã£o cancelada.' };
        }
      }
      ...
      const msg = [
        ...
        '',
        'Piloto: ' + pilot,
        'HorÃ¡rio sugerido: ' + now.hhmm + ' BSB',
        (geo && geo.hasFence) ? (geo.hint || 'Geofence validada por coordenadas.') : 'Dica: vocÃª tambÃ©m pode confirmar pelo horÃ¡rio padrÃ£o das 08:00.'
      ].join('\n');

      const ok = await window.flightAppConfirm(msg, {
        title: 'Jornada DiÃ¡ria',
        okText: 'Iniciar',
        cancelText: 'Ignorar'
      });
      ...
      const entered = await window.flightAppPrompt('Informe o horÃ¡rio de inÃ­cio da jornada (HH:MM):', now.hhmm, {
        title: 'Jornada DiÃ¡ria',
        okText: 'Salvar',
        cancelText: 'Cancelar',
        inputType: 'text',
      ...
      if (!valid) {
        if (window.M) M.toast({ html: 'HorÃ¡rio invÃ¡lido. Use HH:MM', classes: 'orange' });
        return { prompted: true };
      }
      ...
      _dutySavePayload_({
        ...
        status: 'DUTY_OPEN',
        startTime: start,
        summaryText: 'Jornada iniciada Ã s ' + start
      }, function(okSave) {
        if (okSave && window.M) M.toast({ html: 'Jornada iniciada Ã s ' + start, classes: 'green' });
        const cache = _dutyGetCachedPromptState_();
        cache[ctx.dateYmd] = Object.assign({}, cache[ctx.dateYmd] || {}, { morningPrompted: true });
        _dutySetCachedPromptState_(cache);
      });
    ...
    async function _dutyPromptEveningClose_(ctx, snapshot, geo) {
      ...
      _dutySavePayload_({
        ...
        if (okSave && window.M) {
          M.toast({ html: 'Jornada encerrada Ã s ' + now.hhmm, classes: 'green' });
        }
      });
      return { prompted: true };
    }
          ...
          try {
            ...
            if (_dutyCanRunByTime_('morning', now) && snapshot.shouldMorningPrompt && !todayState.morningPrompted) {
              ...
              if (morningRes && morningRes.blocked) {
                _dutyWarnGeofenceOnce_(context.dateYmd, 'morningGeofenceWarned', morningRes.message || 'Geofence nÃ£o validada para prompt matinal.');
              }
              finish();
              return;
            }
            ...
            if (_dutyCanRunByTime_('evening', now) && snapshot.shouldEveningPrompt && !todayState.eveningPrompted) {
              ...
              if (eveningRes && eveningRes.blocked) {
                _dutyWarnGeofenceOnce_(context.dateYmd, 'eveningGeofenceWarned', eveningRes.message || 'Geofence nÃ£o validada para prompt de encerramento.');
              }
              finish();
              return;
            }
    ...
    function updateOfflineCacheStatus(options) {
      ...
      if (state === 'syncing') {
        ...
        if (missionLabel) missionLabel.textContent = stageText || `Downloading ${progress}/${total || 'â€¦'}`;
      ...
      if (state === 'empty') {
        ...
        if (missionLabel) missionLabel.textContent = `Stale Â· ${formatSyncTime(syncedAt)}`;
    ...
    // ============================================================
    // OFFLINE MAP PACK ENGINE â€” available from startup, Tab 1+
    // ============================================================
    (function() {
      ...
      function _renderPackStatusFromMeta(meta) {
        ...
        if (ageDays > MAP_PACK_STALE_DAYS) {
          els.forEach(function(el){ el.textContent='âœ– MAPS EXPIRED â€” last download '+(dateLabel||'unknown date'); el.className='pack-status pack-expired'; });
          _setOfflineMapDbIndicator('expired', 'Expired');
        } else {
          els.forEach(function(el){ el.textContent='âœ” MAPS DOWNLOADED â€” '+(dateLabel||'date unavailable'); el.className='pack-status pack-ok'; });
          _setOfflineMapDbIndicator('ok', 'Ready â€” '+dateLabel);
        }
      }
      ...
      // Main download function â€” works from any tab
      window.downloadMapPack = async function(scope) {
        if (_packDownloadActive) {
          if (window.M) M.toast({ html: 'Maps downloading â€” see progress below', classes: 'blue darken-2' });
          return;
        }
        ...
        // For route scope we need Tab 6 waypoints â€” fall back to base if not available
        let bboxes;
        ...
        if (!bboxes.length) { if (window.M) M.toast({ html: 'No airport data loaded yet â€” sync missions first', classes: 'orange' }); return; }
        ...
        _setOfflineMapDbIndicator('syncing', 'Downloadingâ€¦');
        ...
        ['enroute-pack-btn','tab1-map-pack-btn'].forEach(function(id){ const el=_progEl(id); if(el){ el.textContent='DOWNLOADINGâ€¦'; el.disabled=true; } });
    ...
    function _maybeRefreshMapPackNow_() {
      // Maps are only downloaded manually via the â†º MAPS button.
      // Just refresh the displayed status here â€” never auto-download.
      if (typeof window.refreshOfflineMapPackStatus === 'function') {
        window.refreshOfflineMapPackStatus();
      }
    }
      ...
      const downloadScheduledMissions_ = function() {
        ...
        google.script.run
          .withSuccessHandler(function(missions) {
            ...
            // Cache all aircraft docs for all tails â€” works offline, survives last-minute aircraft switch
            google.script.run
              ...
              .withFailureHandler(function() { /* silent â€” docs cache best-effort */ })
    ...
    // â”€â”€ Garmin .fpl XML builder â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Save .fpl to Drive, return promise({ok, downloadUrl}) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    function _offfltParseRouteTokens(routeStr) {
      ...
      if (!raw) return [];
      const normalized = raw.replace(/[â†’>]/g, ',').replace(/\n|\r/g, ',');
    ...
    function _offfltRecompute() {
      ...
      _offfltSetCalcValue('offflt_calc_dist', distNm > 0 ? `${distNm.toFixed(1)} nm` : 'â€” nm', '#0b5394');
      _offfltSetCalcValue('offflt_calc_time', suggestedTime > 0 ? `${suggestedTime.toFixed(1)} hr` : 'â€” hr', '#0b5394');
      _offfltSetCalcValue('offflt_calc_fuel', requiredFuel > 0 ? `${Math.round(requiredFuel)} L` : 'â€” L', '#0b5394');
      _offfltSetCalcValue('offflt_calc_duty', estDuty > 0 ? `${estDuty.toFixed(1)} hr` : 'â€” hr', estDuty > 14 ? '#c62828' : '#0b5394');
      _offfltSetCalcValue('offflt_calc_route', routePlan.routeText || 'â€”', '#455a64');
    ...
    function openOfflineFlightModal() {
      ...
      if (airportsDl) {
        ...
        airportsDl.innerHTML = airports.map(a => {
          ...
          return `<option value="${icao}">${icao}${name ? ' â€” ' + name : ''}</option>`;
        }).filter(Boolean).join('');
      }
        ...
        if (consumeCamTopLaunchRequest_()) {
          setTimeout(function() {
            ...
            try {
              ...
              if (window.M) M.toast({ html: 'Falha ao abrir inclinÃ´metro automaticamente.', classes: 'orange' });
            }
          }, 450);
        }
      };
      ...
      const requestStartupData_ = function(attempt) {
        ...
        google.script.run.withSuccessHandler(data => {
          ...
          if (isTransientStartupError_(err) && attempt < 4) {
            ...
            if (attempt === 0) M.toast({ html: 'Startup network hiccup. Retryingâ€¦', classes: 'orange', displayLength: 1600 });
            setTimeout(function() {
    ...
    function checkMissionLegality(missionId) {
      ...
      fetchMissionDetails(missionId, mission => {
        ...
        let squawkHtml = '<p class="green-text" style="font-size:0.9rem;">âœ… No open squawks</p>';
              ...
              squawkHtml = visible.map(s => {
                ...
                return `
                  <div style="border-left: 3px solid #fbc02d; padding: 5px 10px; margin-bottom: 5px; background: #fffde7; font-size: 0.85rem;">
                    <b>${dateLabel}:</b> ${s && s.description ? s.description : ''} <i>(${statusLabel}${deferBits.length ? ' â€¢ ' + deferBits.join(' â€¢ ') : ''})</i>
                  </div>
                `;
              }).join('');
            }
          } catch (e) {
            squawkHtml = `<div class="orange-text">âš ï¸ ${openSquawksRaw}</div>`;
          }
        }
        ...
        const tachText = isFinite(currentTach) ? currentTach.toFixed(1) : 'â€”';
        const nextDueText = isFinite(nextDue) ? nextDue.toFixed(1) : 'â€”';
        const hrsText = isFinite(hoursToInsp) ? hoursToInsp.toFixed(1) : 'â€”';
    ...
    function _parseCoordinate(str) {
      ...
      // Normalize Brazilian comma decimal separator (-2,123456 â†’ -2.123456)
      ...
      const norm = s.replace(/[Â°'"]/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    function saveNewWaypoint() {
      ...
      if (isNaN(data.latitude) || data.latitude < -90 || data.latitude > 90) {
        M.toast({ html: 'Latitude: unrecognized format or out of range (Â±90)' });
        return;
      }
      if (isNaN(data.longitude) || data.longitude < -180 || data.longitude > 180) {
        M.toast({ html: 'Longitude: unrecognized format or out of range (Â±180)' });
        return;
      }
    ...
    function initiateWB(missionId) {
      ...
      // Preferred source: already-loaded briefing mission â€” use active leg, not always legs[0]
