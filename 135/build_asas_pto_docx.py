...
def build(out_doc: Path, include_toc: bool) -> None:
    cmd = [
        'pandoc',
        str(src_md),
...
primary_out = Path('135/Asas_PTO.docx')
toc_out = Path('135/Asas_PTO_with_toc.docx')

build(primary_out, include_toc=False)
build(toc_out, include_toc=True)

print(primary_out)
print(primary_out.stat().st_size)
print(toc_out)
print(toc_out.stat().st_size)
