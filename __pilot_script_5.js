...
function wbEnsureInputId_(inputEl) {
  ...
  const autoId = 'wb-num-' + Date.now() + '-' + Math.floor(Math.random() * 10000);
  inputEl.id = autoId;
  return autoId;
}
...
window.wbKeypadPress_ = function(val) {
  ...
  if (val === 'BKSP') {
    ...
    if (opts.decimal && !cur.includes('.')) displayEl.textContent = cur ? (cur + '.') : '0.';
  } else {
    var maxLen = opts.maxLen || 10;
    if (cur.length < maxLen) displayEl.textContent = cur + val;
  }
};
...
function wbKeypadLabelForInput_(inputEl) {
  ...
  if (inputEl.dataset && inputEl.dataset.keypadLabel) return inputEl.dataset.keypadLabel;
  const field = inputEl.closest('.wb-field');
  const fieldLabel = field && field.querySelector('.wb-field-label');
...
function wbOpenNumericKeypadForInput_(inputEl) {
  ...
  const label = wbKeypadLabelForInput_(inputEl);

  if (typeof window.wbKeypadOpen_ === 'function') {
    window.wbKeypadOpen_(inputEl, label, {
      decimal: true
    });
    return true;
  }
  ...
  if (typeof window.flightDeckOpenNumericKeypad === 'function') {
    const inputId = wbEnsureInputId_(inputEl);
  ...
  if (typeof window.briefOpenKeypad_ === 'function') {
    window.briefOpenKeypad_(inputEl, label, {
      decimal: true
    });
    return true;
  }
...
function wbBindNumericKeypad_() {
  ...
  document.addEventListener('pointerdown', function(e) {
    const input = e.target && e.target.closest ? e.target.closest('#wb-container input[type="number"]') : null;
  ...
  document.addEventListener('focusin', function(e) {
    const input = e.target && e.target.matches && e.target.matches('#wb-container input[type="number"]') ? e.target : null;
    if (!input || input.disabled || input.dataset.keypad === 'off') return;
    if (wbOpenNumericKeypadForInput_(input)) {
      input.blur();
    }
...
function wbBuildOfflinePayload_(flightId) {
  ...
  return {
    ...
    route: `${String(leg.from || '').trim().toUpperCase()}GåÆ${String(leg.to || '').trim().toUpperCase()}`,
    airframeData: {
...
window.updateWBUI = function() {
  ...
  if (document.getElementById('wb-route')) {
    ...
    if (!routeText && window.currentBriefingMission && Array.isArray(window.currentBriefingMission.legs) && window.currentBriefingMission.legs.length) {
      ...
      routeText = `${String(firstLeg.from || '').trim().toUpperCase()}GåÆ${String(firstLeg.to || '').trim().toUpperCase()}`;
    }
  ...
  if (!hasRealEnvelope) {
    ...
    envelopeEl.textContent = 'G£ù NO REAL ENVELOPE DATA GÇö FLIGHT MUST NOT PROCEED';
  } else if (isSafe) {
    envelopeEl.className = 'envelope-status safe';
    envelopeEl.textContent = 'G£ô WITHIN ENVELOPE';
  } else {
    envelopeEl.className = 'envelope-status unsafe';
    envelopeEl.textContent = 'G£ù OUTSIDE ENVELOPE - CG or Weight Limit Exceeded';
  }
...
window.renderManifest = function() {
  ...
  installedSeats.forEach(({ seatId, seat }) => {
    ...
    if (seat.passenger && seat.passenger.name) {
      nameHTML += `<br><small style="color:#666; margin-left:8px;">Göö ${seat.passenger.name}</small>`;
    } else if (!seat.locked) {
      nameHTML += `<br><small style="color:#999; margin-left:8px;">Göö Empty</small>`;
    }
  ...
  availableCargoAreas.forEach(area => {
    ...
    if (maxExceeded) {
      areaLabel += ` <span style="color:#d32f2f; font-weight:bold; font-size:1.26rem;">GÜán+Å EXCEEDS MAX</span>`;
    }
        ...
        <div>
          <input type="number"
                 ...
                 onchange="
                   ...
                   if (max > 0 && val > max) {
                     M.toast({html: 'GÜán+Å Exceeds max weight of ' + Math.round(max) + 'kg', displayLength: 3000});
  ...
  baseSeats.forEach(({ seatId, seat }) => {
    ...
    let nameHTML = `<b>${seatId}</b> <small style="color:#999; font-weight:normal;">(${seatWeight}kg)</small>`;
    nameHTML += `<br><small style="color:#999; margin-left:8px;">Göö Left at base</small>`;
  ...
  if (typeof window.runOrQueueServerAction === 'function') {
    window.runOrQueueServerAction({
      ...
      onFailure: function(err) {
        ...
        if (!silent && window.M) M.toast({ html: 'Failed to save W&B GÇö check console', classes: 'red' });
      }
    });
    return;
  }
...
function applyWBPayload_(data) {
  ...
  const missionRoute = (window.currentBriefingMission && Array.isArray(window.currentBriefingMission.legs) && window.currentBriefingMission.legs.length)
    ? `${String(window.currentBriefingMission.legs[0].from || '').trim().toUpperCase()}GåÆ${String(window.currentBriefingMission.legs[0].to || '').trim().toUpperCase()}`
    : '';
