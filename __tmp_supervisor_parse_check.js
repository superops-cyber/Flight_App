    ...
    function _supervisorEnsureLeaflet_(onReady) {
      ...
      if (!document.getElementById('supervisor-leaflet-css')) {
        const link = document.createElement('link');
        link.id = 'supervisor-leaflet-css';
        link.rel = 'stylesheet';
        link.href = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css';
        document.head.appendChild(link);
      }
      const existing = document.getElementById('supervisor-leaflet-js');
      if (existing) {
      ...
      script.id = 'supervisor-leaflet-js';
    ...
    function _supervisorMissionResolveLocation_(token) {
      ...
      const ap = airports.find(function(a) {
        return String((a && a.icao) || '').trim().toUpperCase() === code;
      });
      if (ap) {
      ...
      const wp = waypoints.find(function(w) {
        const ident = String((w && (w.wp_id || w.WP_ID || w.ident || w.Ident || w.fix || w.FIX)) || '').trim().toUpperCase();
        return ident === code;
      });
      if (wp) {
        const lat = parseCoord(wp.lat || wp.latitude || wp.LAT);
        const lon = parseCoord(wp.lon || wp.longitude || wp.LON);
        if (isFinite(lat) && isFinite(lon)) {
          return { key: code, name: code, lat: lat, lon: lon, isAirport: false };
        }
      }
      return null;
    }
    ...
    function _supervisorMissionRoutePoints_(legs) {
      const out = [];
      (Array.isArray(legs) ? legs : []).forEach(function(leg) {
        ...
        tokens.forEach(function(tok) {
          const loc = _supervisorMissionResolveLocation_(tok);
          if (!loc) return;
          out.push(loc);
        });
      });
    ...
    function renderSupervisorMissionMap_(legs) {
      ...
      _supervisorEnsureLeaflet_(function() {
    ...
    function renderDetails(data) {
      ...
      const missionDescription = String((m.meta && m.meta.flightDescription) || '').trim();
      const missionRequests = Array.isArray(m.meta && m.meta.communicationRequests)
        ? m.meta.communicationRequests
        : [];
      const openRequestCount = missionRequests.filter(function(req) {
        return String(req && req.status || '').trim().toUpperCase() !== 'CLOSED';
      }).length;
      ...
      container.innerHTML = `
        ${missionDescription ? `<div style="margin-bottom:10px; padding:10px 12px; border-left:4px solid #6d4c41; background:#fdf7f2; border-radius:4px;"><div style="font-size:0.72rem; text-transform:uppercase; letter-spacing:0.04em; color:#6d4c41; font-weight:800;">Dispatch Flight Description</div><div style="font-size:0.9rem; color:#4e342e; font-weight:700; margin-top:2px;">${missionDescription}</div></div>` : ''}
        ${openRequestCount ? `<div style="margin-bottom:12px; padding:8px 10px; border:1px solid #90caf9; background:#e3f2fd; border-radius:4px; font-size:0.78rem; color:#0d47a1;"><b>${openRequestCount}</b> open pilot communication request(s) on this mission.</div>` : ''}
          ...
          <div style="margin:8px 0 10px; padding:8px 10px; border:1px solid #ef9a9a; border-left:4px solid #c62828; background:#ffebee; border-radius:4px;">
            <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; flex-wrap:wrap;">
              ...
              <div style="display:flex; gap:6px; align-items:center;">
                <button class="btn-small amber darken-3 z-depth-0" style="font-size:0.68rem; font-weight:800;" onclick="askPilotRunwaySurvey_(${idx})">ASK PILOT TO SURVEY RUNWAY</button>
      ...
      ` : '';
        ...
        container.innerHTML += `
          <div class="leg-card" style="${leg.isOver ? 'border-left-color: #d32f2f; background: #ffebee;' : ''}">
            ...
            ${missionDescription ? `<div style="font-size:0.76rem; color:#5d4037; background:#fdf7f2; border-left:3px solid #8d6e63; padding:5px 8px; margin-bottom:8px;"><b>Description:</b> ${missionDescription}</div>` : ''}
    ...
    function askPilotRunwaySurvey_(legIdx) {
      if (!lastLoadedData || !lastLoadedData.mission || !Array.isArray(lastLoadedData.mission.legs)) return;
      const mission = lastLoadedData.mission;
      const leg = mission.legs[Number(legIdx)] || null;
      if (!leg) return;

      const question = 'Are you sure the pilot has enough info to safely land at this location without adding more info now?\n\nOK = send request to pilot now\nCancel = open briefing card to add runway information first.';
      const shouldSendNow = window.confirm(question);
      if (!shouldSendNow) {
        openSupervisorRunwayBriefing_(legIdx);
        return;
      }

      const message = 'Supervisor requested runway survey for leg ' + (Number(legIdx) + 1) + ' (' + String(leg.from || '?') + ' -> ' + String(leg.to || '?') + '). Please survey this runway before release.';
      const payload = {
        type: 'SURVEY_RUNWAY',
        missionId: String(mission.id || ''),
        flightLegId: String(leg.flightLegId || ''),
        legIndex: Number(legIdx),
        from: String(leg.from || '').trim().toUpperCase(),
        to: String(leg.to || '').trim().toUpperCase(),
        message: message,
        createdAt: new Date().toISOString(),
        createdBy: String((window.user && window.user.name) || 'Supervisor').trim()
      };

      google.script.run
        .withSuccessHandler(function() {
          if (window.M) M.toast({ html: 'Pilot survey request added to mission communications.', classes: 'green' });
          if (supMissionId) {
        ...
        .withFailureHandler(function(err) {
          if (window.M) M.toast({ html: 'Failed to save pilot request: ' + (err && err.message ? err.message : String(err)), classes: 'red' });
        })
        .addSupervisorMissionRequest(mission.id, payload);
    }
      ...
      google.script.run
        .withSuccessHandler(function(resp) {
          ...
          list.innerHTML = items.map(function(item) {
            ...
            const runwayClassDefault = String(runwaySnapshot.runwayClass || dbVerified.runwayClass || '').trim();
            return `
                  ...
                  <div style="display:flex; flex-direction:column; gap:6px; min-width:200px;">
                    ...
                    ${isApproval ? `
                      <div style="display:flex; gap:6px; align-items:center; flex-wrap:wrap;">
                        ...
                        <input id="survey-cutdown-${item.stagingId}" type="number" inputmode="numeric" min="1" step="1" value="${runwaySnapshot.cutdownAreaM != null ? Math.round(Number(runwaySnapshot.cutdownAreaM) || 0) : ''}" placeholder="Cutdown m" style="width:110px; height:32px; border:1px solid #ccc; border-radius:6px; padding:0 8px; font-size:0.78rem;">
                      </div>
                    ` : ''}
                    <div style="display:flex; gap:6px; align-items:center; flex-wrap:wrap;">
                    ...
                    <div style="display:flex; gap:6px; justify-content:flex-end;">
                      ...
                      ${!isApproval ? `<button class="btn-small amber darken-3" onclick="openRunwayBriefingFromPendingSurvey('${item.stagingId}')">Briefing Card</button>` : ''}
    ...
    window.openRunwayBriefingFromPendingSurvey = function(stagingId) {
      const list = document.getElementById('runway-survey-review-list');
      if (!list || !stagingId) return;
      ...
      const payload = card ? decodeURIComponent(card.dataset.previewPayload || '') : '';
      if (!payload) {
        if (window.M) M.toast({ html: 'Briefing card data unavailable', classes: 'orange' });
        return;
      }
      ...
      try { item = JSON.parse(payload); } catch (e) { item = null; }
      if (!item) {
        if (window.M) M.toast({ html: 'Failed to parse briefing card payload', classes: 'red' });
        return;
      }
      if (String(item.entryKind || '').toUpperCase() !== 'GPS_SURVEY') {
        if (window.M) M.toast({ html: 'Briefing compare opens for GPS survey requests', classes: 'orange' });
        return;
      }

      const dbAirport = item.dbAirport && typeof item.dbAirport === 'object' ? item.dbAirport : {};
      const dbKnownRaw = String(dbAirport.KNOWN_FEATURES || dbAirport.FEATURES || '').trim();
      let dbKnown = {};
      try { dbKnown = dbKnownRaw ? JSON.parse(dbKnownRaw) : {}; } catch (e) { dbKnown = {}; }
      if (Array.isArray(dbKnown)) dbKnown = { features: dbKnown };
      ...
      const forcedRunway = {
        ...
        rwyIdent: String(item.rwyIdent || '').trim().toUpperCase(),
        length: Number(dbAirport.runwayLength || dbAirport.length || dbVerified.lengthM || 0),
        width: Number(dbAirport.runwayWidth || dbAirport.width || dbVerified.widthM || 0),
        surface: String(dbAirport.surface || dbAirport.runwaySurface || dbVerified.surface || '').trim(),
        elevation: Number(dbAirport.elevation || dbAirport.elevationFt || dbVerified.elevation || 0),
        knownFeaturesRaw: dbKnownRaw,
        verifiedOperational: dbVerified
      };
      ...
      openRunwayBriefingCard(
        forcedRunway.icao,
        forcedRunway.rwyIdent,
        'supervisor',
        forcedRunway,
        {
          pendingSurvey: item.survey || {},
          activeOperational: dbVerified,
          stagingId: item.stagingId,
